    ...
    ListKeysAware
{
    ...
    public function __construct(
        ...
        $this->bucket = $bucket;
        isset ($options['host']) ? $this->hostUrl = $options['host'] : $this->hostUrl = sprintf("https://%s.storage.googleapis.com/", $this->bucket);
     ...
     */
    function getURL($filename, $expires = null)
    {
        $expiresTime = time() + $expires;
        $to_sign = ("GET\n\n\n" . $expiresTime . "\n/" . $this->bucket . '/' . $filename);
        $fp = file_get_contents($this->options['key_location']);
        $googleSigner = new Google_Signer_P12($fp, 'notasecret');
        $signature = $googleSigner->sign($to_sign);
        $signature = urlencode(base64_encode($signature));

        $fileUrl = $this->hostUrl . $filename;

        return $expires ? $fileUrl . '?GoogleAccessId=' . $this->options['account_name'] . '&Expires=' . $expiresTime . '&Signature=' . $signature : $fileUrl;
