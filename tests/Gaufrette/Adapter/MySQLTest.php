...
class MySQLTest extends \PHPUnit_Framework_TestCase
{
    protected function getAdapter()
    {
        ...
        try {
            $adapter = new \PDO('mysql://host=localhost;dbname=gaufrette_test', 'root', 'ahb509');
        } catch (\Exception $e) {
            echo $e->getMessage();
            $this->markTestSkipped('Unable to connect to "localhost"');
        }

        try {
            $adapter->query('DELETE FROM files');
